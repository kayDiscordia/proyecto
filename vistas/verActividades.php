...
try {
    ...
    $empleados = $empleadoController->obtenerEmpleados($idDepartamentoUsuario, true);
...
foreach ($actividades as &$actividad) {
    $actividad['archivosAdjuntos'] = $actividadController->obtenerArchivosPorActividad($actividad['idActividad']);
}
unset($actividad);
...
<head>
    ...
    <style>
        ...
        #contenedorArchivosAdjuntos a:hover {
            background-color: #f0f7ff;
            border-radius: 0.25rem;
        }
                                    ...
                                    <?php foreach ($actividades as $index => $actividad): ?>
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                                                <?= date('d-m-Y H:i', strtotime($actividad['fechaCreacion'])) ?>
                                            </td>
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <?php
                                                $estadoClases = [
                                                    'Por Iniciar'  => 'bg-[#FACC15] text-white',    // Amarillo
                                                    'Retraso'      => 'bg-[#1D4ED8] text-white',    // Azul
                                                    'En progreso'  => 'bg-[#F97316] text-white',    // Naranja
                                                    'Cancelada'    => 'bg-[#EF4444] text-white',    // Rojo
                                                    'Completada'   => 'bg-[#10B981] text-white',    // Verde
                                                ];
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-center">
                                                <div class="flex justify-center space-x-1">
                                                    ...
                                                    <?php
                                                    // Solo mostrar Culminar y Cancelar si es admin o el empleado asignado
                                                    $puedeAccionar = (
                                                        (isset($_SESSION['idRol']) && $_SESSION['idRol'] == 1) ||
                                                        (isset($_SESSION['id']) && $_SESSION['id'] == $actividad['idEmpleado'])
                                                    );
                                                    ?>
                                                    <?php if (
                                                        $puedeAccionar &&
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#tablaActividades').DataTable({
                        ...
                        customize: function(doc) {
                            ...
                            doc.content.splice(0, 0, {
                                columns: [{
                                    ...
                                    {
                                        text: 'Centro de Desarrollo para la Calidad Educativa',
                                        fontSize: 16,
                                        alignment: 'center',
                                        margin: [10, 15, 0, 12]
                                    }
                ...
                columnDefs: [{
                        orderable: false,
                        targets: [8], // Cambia a 8 si la columna de acciones es la última
                        className: 'dt-center'
                    },
                    {
        ...
        function mostrarDetalles(actividad) {
            let archivosHtml = '';
            if (actividad.archivosAdjuntos && actividad.archivosAdjuntos.length > 0) {
                archivosHtml += `<div class="mt-2"><strong>Archivos Adjuntos:</strong><ul style="margin-top:5px;">`;
                actividad.archivosAdjuntos.forEach(archivo => {
                    archivosHtml += `
            ...
            Swal.fire({
                title: `Detalles de: ${actividad.nombreActividad}`,
                html: `
        <div class="text-left">
            <p><strong>Descripción:</strong> ${actividad.descripcionActividad || ''}</p>
            <p><strong>Fecha Inicio:</strong> ${actividad.fechaInicio || ''}</p>
            <p><strong>Fecha Culminación:</strong> ${actividad.fechaCulminacion || ''}</p>
            <p><strong>Empleado:</strong> ${actividad.nombreEmpleado || ''}</p>
            <p><strong>Categoría:</strong> ${actividad.categoriaActividad || ''}</p>
            <p><strong>Estado:</strong> ${actividad.estadoActividad || ''}</p>
            ${(actividad.estadoActividad === 'Cancelada' && actividad.descripcionCancelacion) ? `<p><strong>Motivo de Cancelación:</strong> ${actividad.descripcionCancelacion}</p>` : ''}
            ${(actividad.estadoActividad === 'Completada' && actividad.descripcionCulminacion) ? `<p><strong>Descripción de Culminación:</strong> ${actividad.descripcionCulminacion}</p>` : ''}
            ${archivosHtml}
        </div>
    `,
                width: 600,
        ...
        function mostrarEditar(actividad) {
            let empleadosOptions = '';
            <?php foreach ($empleados as $empleado): ?>
                empleadosOptions += `<option value="<?= $empleado['idEmpleado'] ?>"><?= htmlspecialchars($empleado['nombres'] . ' ' . $empleado['apellidos']) ?></option>`;
            <?php endforeach; ?>
        ...
        <form id="formEditarSwal">
            ...
            <input type="date" id="editarFechaInicio" class="swal2-input" value="${actividad.fechaInicio || ''}" min="${fechaHoy}" required>
        ...
        `,
                ...
                preConfirm: () => {
                    ...
                    return {
                        ...
                        fechaCulminacion: document.getElementById('editarFechaCulminacion').value,
                        idEmpleado: document.getElementById('editarEmpleado').value,
        ...
        function mostrarCulminar(actividad) {
            Swal.fire({
                title: `Culminar: ${actividad.nombreActividad}`,
                html: `
            <form id="formCulminarSwal">
            <label for="culminarDescripcion" class="block text-sm font-medium text-gray-700 mb-1" style="margin-bottom:8px;">Descripción de Culminación</label>
            <textarea id="culminarDescripcion" rows="5" style="width:100%;min-width:320px;max-width:100%;min-height:90px;resize:vertical;padding:10px;border-radius:8px;border:1px solid #d1d5db;font-size:15px;" required></textarea>
        </form>
        `,
                showCancelButton: true,
                ...
                preConfirm: () => {
                    return {
                        idActividad: actividad.idActividad,
                        descripcionCulminacion: document.getElementById('culminarDescripcion').value
                    };
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    fetch('formularioCulminar.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        },
        ...
        function mostrarCancelar(actividad) {
            Swal.fire({
                title: `Cancelar: ${actividad.nombreActividad}`,
                html: `
           <form id="formCancelarSwal">
            <label for="cancelarDescripcion" class="block text-sm font-medium text-gray-700 mb-1" style="margin-bottom:8px;">Motivo de Cancelación</label>
            <textarea id="cancelarDescripcion" rows="5" style="width:100%;min-width:320px;max-width:100%;min-height:90px;resize:vertical;padding:10px;border-radius:8px;border:1px solid #d1d5db;font-size:15px;" required></textarea>
        </form>
        `,
                showCancelButton: true,
                ...
                preConfirm: () => {
                    return {
                        idActividad: actividad.idActividad,
                        descripcionCancelacion: document.getElementById('cancelarDescripcion').value
                    };
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    fetch('formularioCancelar.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        },
