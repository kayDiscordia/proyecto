...
<body class="bg-[#E8EEFF]">
        ...
        <main class="flex-1 p-6 overflow-y-auto bg-e8eeff">
            ...
            <div id="modalDetalles" class="fixed inset-0 items-center justify-center hidden">
                <div class="bg-white p-6 rounded-lg shadow-lg w-1/2">
                    ...
                    <div id="seccionArchivosAdjuntos" class="mt-4">
                        <strong>Archivos Adjuntos:</strong>
                        <div id="contenedorArchivosAdjuntos" class="mt-2 space-y-2"></div>
                    </div>
    ...
    <script>
        ...
        function mostrarDetalles(actividad) {
            ...
            const contenedorArchivos = document.getElementById('contenedorArchivosAdjuntos');
            contenedorArchivos.innerHTML = '';
            if (actividad.archivosAdjuntos && actividad.archivosAdjuntos.length > 0) {
                document.getElementById('seccionArchivosAdjuntos').classList.remove('hidden');
                actividad.archivosAdjuntos.forEach(archivo => {
                    const elementoArchivo = document.createElement('div');
                    elementoArchivo.className = 'mt-2 p-2 border rounded flex justify-between items-center';
                    const enlaceArchivo = document.createElement('a');
                    enlaceArchivo.href = '../archivos/' + archivo.rutaArchivo;
                    enlaceArchivo.target = '_blank';
                    enlaceArchivo.className = 'text-blue-600 hover:text-blue-800 flex items-center';
                    enlaceArchivo.download = archivo.nombreArchivo;
                    enlaceArchivo.innerHTML = `
                        <i class="fas fa-file-download mr-2"></i>
                        <span>${archivo.nombreArchivo}</span>
                    `;
                    const botonDescargar = document.createElement('a');
                    botonDescargar.href = '../archivos/' + archivo.rutaArchivo;
                    botonDescargar.download = archivo.nombreArchivo;
                    botonDescargar.className = 'ml-4 px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-700';
                    botonDescargar.innerHTML = '<i class="fas fa-download"></i> Descargar';
                    elementoArchivo.appendChild(enlaceArchivo);
                    elementoArchivo.appendChild(botonDescargar);
                    contenedorArchivos.appendChild(elementoArchivo);
                });
            } else {
                document.getElementById('seccionArchivosAdjuntos').classList.add('hidden');
            }
        ...
        function mostrarEditar(actividad) {
            ...
            // Limitar fecha de inicio: hoy o mayor
            const hoy = new Date();
            const hoyStr = hoy.toISOString().split('T')[0];
            fechaInicioInput.min = hoyStr;

            // Limitar fecha de fin: igual o mayor a fecha de inicio
            fechaFinInput.min = fechaInicioInput.value;

            // Cuando cambia la fecha de inicio, actualizar el mínimo de fecha de fin
            fechaInicioInput.addEventListener('input', function() {
                fechaFinInput.min = this.value;
                // Si la fecha de fin es menor, ajustarla automáticamente
                if (fechaFinInput.value < this.value) {
                    fechaFinInput.value = this.value;
                }
            });

            // Cuando cambia la fecha de fin, si es menor a inicio, ajustarla
            ...
            // Seleccionar empleado y categoría
