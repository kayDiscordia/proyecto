            ...
            <div class="bg-white p-6 rounded-lg shadow">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-sm text-gray-500">
                        <?= count($listaEmpleados) ?> empleado(s) encontrado(s)
                    </span>
                </div>
                ...
                <?php if (empty($listaEmpleados)): ?>
                    <div class="text-center py-2">
                        ...
                        <p class="mt-2 text-gray-500">No hay empleados registrados.</p>
                    </div>
                <?php else: ?>
                    <div class="rounded-lg shadow bg-white" style="max-width: 100%;">
                        <div style="max-height: 420px; overflow-y: auto;">
                            <table id="tablaEmpleados" class="min-w-full text-xs">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    ...
                                    foreach ($listaEmpleados as $index => $datos): ?>
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700"><?= htmlspecialchars($datos['nombres'] ?? '') ?></td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700"><?= htmlspecialchars($datos['apellidos'] ?? '') ?></td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700"><?= htmlspecialchars($datos['cedula'] ?? '') ?></td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700"><?= htmlspecialchars($datos['cargo_nombre'] ?? '') ?></td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700"><?= htmlspecialchars($datos['departamento_nombre'] ?? '') ?></td>
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <span class="px-2 py-1 rounded-full text-xs font-semibold
                                                    <?= 
                                                    ($datos['estado_nombre'] == 'Activo') ? 'bg-green-100 text-green-800' : 
                                                    (($datos['estado_nombre'] == 'Suspendido') ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800')
                                                    ?>">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-center">
                                                <div class="flex justify-center space-x-1">
                                                    <a href="modificarEmpleado.php?idEmpleado=<?= $datos['idEmpleado'] ?>" class="text-yellow-600 hover:text-yellow-900 p-1 rounded-full hover:bg-yellow-50" title="Editar">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a href="verReportes.php?idEmpleado=<?= $datos['idEmpleado'] ?>" class="text-blue-600 hover:text-blue-900 p-1 rounded-full hover:bg-blue-50" title="Ver reportes">
        ...
        $(document).ready(function() {
            $('#tablaEmpleados').DataTable({
                ...
                buttons: [{
                        ...
                        title: 'Lista de Empleados - <?= date("d-m-Y") ?>',
                        exportOptions: {
                ...
                columnDefs: [{
                    ...
                    },
                    {
                        ...
                        targets: [0, 1, 6] // Prioridad para columnas importantes en modo responsive
                    }
                ],
